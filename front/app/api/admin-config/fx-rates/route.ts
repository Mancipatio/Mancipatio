...
// older than its max age. Decimals are read from chain, never trusted.
...
import { paymentMintDecimals } from "@/app/api/sale-approvals/_lib";
  ...
  try {
    ...
    if (isWrite) {
      ...
      if (params.op === "delete") {
        ...
        const decimals = await paymentMintDecimals(mint);
