// POST /api/conversion/cancel — holder cancels their own request pre-deposit.
...
// cancellations (after the vault opened) are an admin action. Mirror of
// /api/delivery/cancel.
...
export async function POST(request: Request) {
  try {
    const { wallet, params } = await verifySigned(request, "conversion.cancel");
    ...
    if (!row) throw new SiwsError(404, "Conversion request not found");
    ...
    if (error) {
      console.error("[api/conversion/cancel] update failed:", error.message);
