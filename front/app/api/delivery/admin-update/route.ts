// POST /api/delivery/admin-update — admin lifecycle writes on a delivery
// request (open vault, mark in delivery, confirm delivery, reject, return).
// Signed (SIWS) + on-chain admin gate. Status transitions are additionally
// pinned by the DB trigger from migration 0030 — an illegal jump surfaces as
// a 409 here.
...
// 'approved' is dead (dropped from the front); 'requested' is never re-entered.
...
export async function POST(request: Request) {
  try {
    ...
    const previousStatus = await validateCustodyUpdate("delivery_requests", id, patch);
    const { data, error } = await sb
      ...
      .maybeSingle();
    if (error) {
      // The 0030 status-guard trigger raises on illegal transitions.
      const illegal = error.message.includes("illegal delivery status");
      console.warn("[api/delivery/admin-update] update failed:", error.message);
      throw new SiwsError(
        illegal ? 409 : 500,
        illegal ? error.message : "Could not update the delivery request",
      );
    }
    if (!data) throw new SiwsError(404, "Delivery request not found");
