...
select network, mode, interval_seconds, sample_size, reconcile_max_age_hours,
  last_attempt_at, last_outcome, last_reason, last_proven_at, declined_since, last_expired_at,
...
where network = public.deployment_network() and check_key in ('indexer-gap', 'indexer-degraded', 'indexer-freshness')
order by check_key;
rollback;
