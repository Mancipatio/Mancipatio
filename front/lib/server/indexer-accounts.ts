...
type Entry = { table: string; discriminator: ReadonlyUint8Array; decode: (data: Uint8Array) => Promise<Row> };
...
function spec<T>(
  ...
  project: (value: T) => Row, derive: (value: T) => Promise<string>, expectedVersion: number | readonly number[] = 1,
): Entry {
  return { table, discriminator, decode: async (bytes) => {
    ...
    return { ...project(value), pda: await derive(value), account_version: version ?? 0 };
  } };
}
...
export const INDEXER_ENTITIES: readonly Entry[] = [
  ...
  }), (a) => pda([text("kyc_registry"), key(a.authority)])),
...
export async function decodeIndexerAccount(pdaAddress: string, owner: string, bytes: Uint8Array): Promise<DecodedIndexerAccount | null> {
  ...
  const row = await entry.decode(bytes); // generated decoder enforces complete field lengths
