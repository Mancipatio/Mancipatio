...
begin
  ...
  if p_subject like 'spv:%' then
    ...
    select annual_cap_eur into cap from public.spvs where id = spv;
...
begin
  ...
  if p_spv_id is not null then
    perform 1 from public.spvs where id = p_spv_id for update;
    if not found then raise exception 'SPV_NOT_FOUND' using errcode = 'P0001'; end if;
  end if;
  return subject;
end $$;
...
begin
  ...
  if p_max_gross_raise is null or p_max_gross_raise <= 0 or p_min_price_per_unit is null or p_min_price_per_unit <= 0
     ...
     or p_expires_at <= now() or p_raise_type not in ('mature','startup') or p_reserved_by is null then
  ...
  insert into public.sale_capacity_reservations(network, kind, share_class_pda, sale_id, approval_pda, sale_pda, asset_pda,
    ...
    max_gross_raise, min_price_per_unit, max_price_per_unit, raise_type, expires_at, amount_eur, fx_rate, fx_kind,
    fx_source, fx_as_of, reserved_by)
  values (p_network, 'sale', p_share_class_pda, p_sale_id, p_approval_pda, p_sale_pda, p_asset_pda, p_issuer_pda, p_spv_id,
    ...
    p_max_gross_raise, p_min_price_per_unit, p_max_price_per_unit, p_raise_type, p_expires_at, amount, fx.eur_per_token,
    fx.kind, fx.source, fx.as_of, p_reserved_by)
...
-- The approval was used by open_sale. Shrinks the counted value to the sale's
-- actual maximum (price * total, at the locked rate); never grows it.
create or replace function public.consume_sale_reservation(p_id uuid, p_sale_pda text, p_sale_gross_max numeric)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare r public.sale_capacity_reservations%rowtype; shrunk numeric;
begin
  select * into r from public.sale_capacity_reservations where id = p_id and kind = 'sale' for update;
  if not found then raise exception 'RESERVATION_NOT_FOUND' using errcode = 'P0001'; end if;
  ...
  if p_sale_gross_max is null or p_sale_gross_max <= 0 or p_sale_gross_max > r.max_gross_raise then
    raise exception 'SALE_EXCEEDS_RESERVATION' using errcode = 'P0001';
  end if;
...
revoke all on function
  ...
  public.reserve_sale_capacity(text,text,numeric,text,text,text,text,uuid,uuid,jsonb,text,text,integer,numeric,numeric,numeric,text,timestamptz,text),
