...
import { ASSET, DONOR, FAKE_MINT_A, FAKE_MINT_B, FakeChainOps, FakeSite, SALES } from "./helpers/sim-fake-site";
...
async function world(plans: UserPlan[], options: { pace?: boolean } = {}): Promise<World> {
  ...
  return { ctx, site, chain, journal, clock, state, users: plans.map((p) => state.users[p.label]), limiter, locks };
}
...
describe("transfers (cohort X, wave 6)", () => {
  ...
  it("C1: a stale first read is ok with its lag; a balance still wrong after the re-reads is a consistency finding", async () => {
    ...
    expect([s2.outcome, s2.body]).toEqual(["ok", "lag 10s"]);
    ...
    expect(bad[0].err).toMatch(/expected u101=1 u102=2, saw 3 and 2 \(still after 3 reads over 20 s\)/);
