...
describe.skipIf(process.env.RUN_LOCAL_POSTGRES_TESTS !== "1")("0047 complete snapshot and durable queue on isolated PostgreSQL", () => {
  beforeAll(async () => {
    try {
      ...
      for (const file of ["0002_indexer.sql", "0014_asset_profiles.sql", "0015_issuer_profiles.sql", "0037_indexer_integrity.sql", "0038_indexer_composite_key.sql", "0039_indexer_events_wallets.sql", "0040_indexer_kyc.sql", "0042_indexer_deposit_ledgers.sql", "0047_indexer_retry.sql", "0064_platform_pause_flags.sql", "0067_sales_sale_approval.sql"]) {
